<template>
  <div class="YunDingComment" ref="containerRef" style="width: 100%; height: 90%;">
...
// 当前显示的年份起点和长度
let startIndex = 0
let displayCount = 5 // 根据宽度自动调整
let autoTimer: any = null

// 请求单个产品数据
const fetchProductData = async (productName: string) => {
  const res = await fetch(`http://106.55.169.134:10010/product/countDate?productName=${encodeURIComponent(productName)}`)
...
async function fetchAll() {
  ...
  // 初始化每个产品在所有年份的值（没有的为0）
  // 可选：如果想确认每年都显示（即没有的为0）
  // 这里不用，只在切片时处理
  setupResizeObserver()
  startAutoPlay()
  updateChart()
