...
const fetchData = async () => {
  try {
    ...
    // 筛选出医师的数据
    const doctorData = data.filter((item: { job: string }) => item.job === '医师');

    // 提取工作经验和数量
    const experience = doctorData.map((item: { workExperience: string }) => item.workExperience);
